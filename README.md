...
- **Music Playback:** Play music from YouTube in the voice channel. Say `play [query] on youtube` or `play [query] song` to play a song. You can also use the `>play` command.
- **Timers:** Set a timer by saying `set a timer for [time]` or `set an alarm for [time]`. The bot will notify you when the timer is up.
- **Internet search:** Ask the bot to search the internet for you by saying `search [query] on internet` or `search on internet for [query]`. The bot will respond using the web.
- **Vision:** Send an image mentioning the bot, and it will react to it in voice chat.
...
**You may at any time say `stop` to stop the bot while it is speaking.**
